	...
	for _, event := range events {
		for _, notify := range event.Notify {
			if notify.ContractAddress == this.polyCfg.CCMContract {
				...
				if uint64(fchainid) == basedef.ETHEREUM_CROSSCHAIN_ID || uint64(fchainid) == basedef.BSC_CROSSCHAIN_ID || uint64(fchainid) == basedef.HECO_CROSSCHAIN_ID || uint64(fchainid) == basedef.O3_CROSSCHAIN_ID || uint64(fchainid) == basedef.MATIC_CROSSCHAIN_ID {
